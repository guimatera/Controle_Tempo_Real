...
% Usando par{i}.Cm*par{i}.xm0, e mais para frente no código, escrevi x0+yss
% como C.I.s
x0 = [12; 0];
y0 = [0; 0];
z0 = [1; 0];
...
% ----------------------------------------------------------------
R=1; %multiplica o controle
g=0.5;%0.999919;
Qe=2.5; % multiplica os estados
%Qe=1/10; % For CONTINUOUSdrone flag

%% Initial Gain, Noise and Disturbance
K0factor=1/1; % For CONTINUOUSdrone flag
%K0factor=1;
noise=0;
dc0=0;d0=0;
...
for i=1:4
    switch i
        case 1
            ymtilde=((c1-c2)*cos(c3*ts)+c4*cos(((c1-c2)*c3/c2)*ts));
            [s{i},par{i},buffer{i}]=F.InitializeLQTint(h,xi0,Ac,Bc,Cc,Dc,x0+yss,ymtilde,yss,R,Qe,g,K0factor,THETA0factor,PRLS0factor,noise,d0,dc0);
        case 2
            ymtilde=((c1-c2)*sin(c3*ts)-c4*sin(((c1-c2)*c3/c2)*ts));
            [s{i},par{i},buffer{i}]=F.InitializeLQTint(h,xi0,Ac,Bc,Cc,Dc,y0+yss,ymtilde,yss,R,Qe,g,K0factor,THETA0factor,PRLS0factor,noise,d0,dc0);
        case 3
            c3=3*0.1571/10;
            ymtilde=cos(c3*ts);
            yss=10;
            [s{i},par{i},buffer{i}]=F.InitializeLQTint(h,xi0,Ac,Bc,Cc,Dc,z0+yss,ymtilde,yss,R,Qe*2,g,K0factor,THETA0factor,PRLS0factor,noise,d0,dc0);
        case 4
            c3=3*0.1571/20;
            ymtilde=(45*pi/180)*cos(c3*ts);
            yss=0;
            [s{i},par{i},buffer{i}]=F.InitializeLQTint(h,xi0,Ac,Bc,Cc,Dc,yaw0+yss,ymtilde,yss,R,Qe,g,K0factor,THETA0factor,PRLS0factor,noise,d0,dc0);
